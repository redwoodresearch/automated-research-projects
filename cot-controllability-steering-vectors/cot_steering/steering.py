...
def steering_spec(layers, vectors, scale: float = 1.0):
    """Build the apply-time spec ``[{"layer": L, "vector": [...]}, ...]`` (optionally scaled).

    ``scale=1.0`` reproduces the trained vector; ``0.0`` is the unsteered base model. The vector's
    magnitude is learned by gradient descent, so no per-layer norm rescaling is applied.
    """
    return [{"layer": int(L), "vector": (float(scale) * np.asarray(v, dtype=np.float32)).tolist()}
            for L, v in zip(layers, vectors)]


def vector_norm(vectors) -> float:
    """Total L2 norm across the per-layer vectors (the gL10 deliverable is ~148)."""
    return float(np.sqrt(sum(float(np.linalg.norm(v)) ** 2 for v in vectors)))
