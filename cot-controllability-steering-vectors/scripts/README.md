...
| `build_gsm_symbolic.py` | Build the fresh templated **GSM-Symbolic** tasks (memorization control). |
| `build_faith_targets.py` | Build the cue / faithfulness / reliance target sets for the monitorability arc. |

## evaluation/ — held-out evals + the deliverable analyses (produce fig1 / fig2 data)
| file | what it does |
|---|---|
...
| `analyze_steer_deliverable.py` | **Deliverable #2 analysis** → `steer_deliverable_gL10.json` (fig1 steering arm + fig2 bullet bars): per-instruction + aggregate `effective_control`, cluster-bootstrap CIs, paired vector−FT difference. |
...
| `run_steer_eval.py`, `analyze_steer_eval.py` | Single-layer **diff-of-means** ("average-difference direction") held-out eval → `steer_eval_heldout_analysis.json` (fig2 avg-diff bar, n=39). |
...
## mechanism/ — fig3 / fig4 / fig5 data
| file | what it does |
|---|---|
| `run_qkov_patch.py`, `analyze_qkov.py` | **fig3**: causal patch of the steered attention **pattern** vs **values** → `mech_qkov.json` (pattern ≫ value for bullet & numbered). |
| `run_capture_attn.py` | Capture per-head attention mass onto named instruction spans (base vs steered). |
| `run_tok_subspan.py`, `analyze_tok_subspan.py` | **fig4 / fig5**: per-instruction-part attention (format-specifier / directive / "your reasoning" / rest), base vs steered → `tok_subspan.json` + the per-example `tok_subspan_attn.npz`. |
