"""Segment 15 (mech_tokens_prompting) — P1 analysis: the BROADENED QK-pattern-vs-OV-value verdict +
...
def main():
    ...
    md = ["# P1 (broadened) — QK-pattern vs OV-value verdict + attention-to-instruction knockout\n",
          f"Sample: **{len(np.unique(tasks))} source-stratified held-out tasks** "
          f"(SIZES15={d['sizes']}) × the held-out formatting/casing instructions, vs Seg-11's 12. "
    ...
    md.append("\n**Verdict (broadened):** on the carriers with a clean single-token marker the form "
              ...
              "onto the instruction removes essentially all of it — replicating Seg-11 on ~4× the "
