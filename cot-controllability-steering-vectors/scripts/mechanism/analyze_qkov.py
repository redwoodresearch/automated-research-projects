"""Segment 11 (mech_interp) — P2.5 analysis: the modulation-vs-gated-additive VERDICT
(QK-pattern vs OV-value patching) + mediating-component ablations (P4.7 necessity) + the
meta-suppression channel (P4.8). Reads results/mech_qkov_raw.json. Fast (no Modal).
...
def main():
    ...
    md = ["# P2.5 — which PATHWAY carries the effect: QK-pattern vs OV-value (gating vs additive)\n",
    ...
    # meta-suppression channel (P4.8)
    md.append("## P4.8 — meta-suppression channel: same mechanism as form, or separate?\n")
