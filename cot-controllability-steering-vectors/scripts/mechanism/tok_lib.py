"""Segment 15 (mech_tokens_prompting) — shared scaffolding for the BROADENED attention-gating
verification (P1), the token-level sub-span causal attribution (P2), and the mechanism-guided
prompting eval (P3).

Extends Segment-11's `mech_lib` (which it imports as M): same teacher-forced design (a shared base
...
The cardinal rule (Seg 10/11): an attention map is a HYPOTHESIS; every load-bearing claim is confirmed
...
# 8/source × 6 = 48 tasks for the teacher-forced attribution (vs Seg-11's 12).
...
# ---------------------------------------------------------------------------
# Full instruction-span locator (robust; replaces Seg-11's suffix-diff which can truncate the span
