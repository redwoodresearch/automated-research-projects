"""Segment 11 (mech_interp) — shared scaffolding for mechanistic attribution of gL10.

Builds the teacher-forced contexts (the SAME design as Segment-10's run_induced_shift: a shared
base no-instruction CoT prefix per task, so the ONLY difference across conditions is the prompt
instruction) and the candidate-token set (form markers + meta/persona tokens) that all the P1–P4
analyses attribute. Anchors all mechanistic work to the Segment-10 VALIDATED behavioral readout
...
# Seg-10 1/source set (same shuffle key) so cached base generations re-hit.
