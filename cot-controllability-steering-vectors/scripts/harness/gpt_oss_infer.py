...
class GptOss:
    ...
    def load(self):
        ...
        # stream; an UNQUANTIZED bf16 merge (Priority-2 faithful serving path) has NO mxfp4 config
    ...
    # -- architecture probe (Segment 11 mech_interp — confirm submodule/return structure) -----
    @modal.method()
    def arch_probe(self, sample_prompt_token_ids: list[int]) -> dict:
        """Inspect the gpt-oss decoder-layer internals we will hook for mechanistic attribution
        (Segment 11): per-layer attention type (sliding vs full), the self_attn / mlp / router
    ...
    # -- teacher-forced analysis-span capture (Segment 6 steering directions) ----
    @modal.method()
    ...
    # -- gradient-trained additive steering bias (Segment 7, Priority 1) ----
    @modal.method()
    ...
    # -- greedy generation with sub-block ablation (Seg 11 P4.7 — behavioral spot-check) ----
    @modal.method()
    def gen_ablate(self, request: dict) -> dict:
        ...
        mask_spec (Seg-15 behavioral sub-span knockout) = {layers:[...], heads:"all"|[...],
        ...
        for _step in range(n_new):
            ...
            # Seg-15: surgical attention-sub-span masking during generation
            if mask_layers:
                def make_mask(_L):
    ...
    # -- logit lens / vocab projection of residual-stream directions (Seg 10, P1.1 + P3.6) ----
    @modal.method()
    ...
    # -- actual induced next-token logit shift under steering (Seg 10, P1.2 — the reliable read) --
    @modal.method()
    ...
    # -- mechanistic internals: resid-lens trajectory + sub-block DLA + MoE router (Seg 11 P1-P3) --
    @modal.method()
    ...
    # -- per-head attention mass onto key spans + the sink (Seg 11 P2.4 — the gating test) ----
    @modal.method()
                    ...
                    for p in positions[i]:
                        ...
                        for L in cap_layers:
                            ...
                            for name, v in sr.items():
                                # v is either a contiguous [a,b] (Seg-11) or {"ids":[...]} (Seg-15
                                # sub-spans; possibly disjoint token indices).
    ...
    # -- full attention-weight ROWS for chosen (layer,head) — for eyeballing real maps (Seg 11) --
    @modal.method()
    ...
    # -- QK-pattern vs OV-value patching + sub-block ablation (Seg 11 P2.5 + P4.7) ----
    @modal.method()
    def patch_attn(self, request: dict) -> dict:
        ...
        patch_spec modes:
          ...
          * "expert": in the STEERED run, at the listed (layer, expert) pairs, FORCE-DROP or FORCE-IN
            ...
            {mode:"expert", drop:[[layer,expert],...], add:[[layer,expert],...]}  (P3/P4).
                ...
                def make_mask(_L):
                    ...
                    def ohook(module, args, output):
                        ...
                        for r, i in enumerate(idxs):
                            ...
                            if sp is None:
                                continue
                            # sp is either contiguous [a,b] or {"ids":[...]} (Seg-15 sub-spans)
                            if isinstance(sp, dict):
                                col = torch.tensor([int(x) for x in sp["ids"]],
                                                   dtype=torch.long, device=A.device)
                                if col.numel() == 0:
...
# Bumped only when lens_project / induced_shift bodies change (invalidates Seg-10 projections).
LENS_VERSION = 2
...
# v2 (Seg-15): capture_attn / patch_attn(mask_instr) / gen_ablate accept explicit token-index
...
def capture_attn(obj, sequences, positions, steering, span_ranges, cache, tracker, *,
    ...
    def _norm_span(v):
        # contiguous [a,b] (Seg-11) or {"ids":[...]} explicit index list (Seg-15 sub-spans)
        if isinstance(v, dict):
...
def gen_ablate(obj, sequences, steering, ablate_targets, cache, tracker, *, n_new: int = 64,
    ...
    the Seg-15 ``mask_spec`` attention-sub-span knockout)."""
    def _norm_mask(ms):
        if not ms:
            return None
        sr = {}
