...
This repo regenerates the five publication figures from released data artifacts on CPU (no GPU, no
...
**values**, and the added attention concentrates on the **format-specifier** tokens (bullet specifier
attention 2.3→6.5). It is the same late-layer attention circuit fine-tuning uses (cosine 0.94). This
is a **reachability** result — the vector needs the same training data + white-box gradient access as
fine-tuning — not a cheap attack. All results are on a single model.

## The five figures (regenerated here)
...
**fig1 — a frozen-weights steering vector reproduces fine-tuning's held-out CoT control**
![fig1](figures/fig1_headline.png)

**fig2 — on held-out *bullet* formatting, only the gradient-trained vector and the fine-tune produce bullets** (the average-difference direction, a random vector, and the base sit at ~0%)
![fig2](figures/fig2_gradient_unlocks_formatting.png)

**fig3 — the effect flows through *where* the heads look (attention pattern), not *what* they read (attention values)**
![fig3](figures/fig3_mechanism.png)

**fig4 — the instruction text shaded by the per-part attention increase once the vector is added** (concentrated on the format specifier)
![fig4](figures/fig4_attention_tokens.png)

**fig5 — attention onto each instruction part, base vs steered** (the format specifier roughly doubles/triples; the "your reasoning" reference does not)
![fig5](figures/fig5_subspan.png)
...
├── generate_figures.py        # entry point: regenerate + (optionally) verify fig1-fig5
├── precompute_figure_data.py  # re-derive figure_data/ from the raw artifacts (CPU, ~$0)
├── cot_steering/              # the minimal, cleaned package
│   ├── figures.py             #   fig1-fig5 plotting (regenerates from the summaries)
...
- `steering_vectors/` — the headline `grad_steer_gL10.npz` (layer 10, 2,880 floats) + the full family
  ...
  gL10rand, multi-layer gML; the faithfulness/concealment + reliance vectors) and the diff-of-means
  directions (`steering_directions.npz`, `ftbase_direction.npz`).
- `datasets/` — the task pool (`tasks_all*.jsonl`), the instruction splits (`instruction_splits.json`),
  the edited-reasoning SFT data + the raw-trace control, the source traces, the plain mix, the
  cue/faithfulness/reliance targets, and the GSM-Symbolic tasks.
- `figure_data/` — the small summary JSONs the figures plot.
- `results_raw/` — the raw artifacts needed to re-derive `figure_data/` (the per-example fig5 attention
  tensors + the per-row judged generations) + the headline judged eval files. (The full ~700 MB of
...
This re-derives `fig5_subspan_attention.json` (bootstrap CIs over per-example attention) and
`fig2_random_null.json` (random-vector bullet `effective_control`) on CPU at ~$0. See
