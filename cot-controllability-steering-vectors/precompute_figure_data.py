...
and the master notebook consume to regenerate fig1-fig5.
...
simply copied through unchanged (they are the canonical, verified artifacts):
  ...
  * ``steer_deliverable_gL10.json``            -- fig1 (steering arm) + fig2 (bullet bars)
  * ``ft_deliverable_cdel_vs_ctrldel.json``    -- fig1 (fine-tune arm)
  * ``steer_eval_heldout_analysis.json``       -- fig2 (average-difference direction, n=39)
  * ``mech_qkov.json``                         -- fig3 (pattern vs value)
  * ``tok_subspan.json``                       -- fig4 (per-token attention deltas)

Two numbers the reference figures need are NOT in any saved summary, so this script
derives them from the raw run artifacts on CPU at ~$0 (no model generation):
...
FIG5_SPANS = ["spec", "cot_target", "directive", "rest"]
FIG5_CONDS = ["bullet", "numbered"]
BOOT_N = 2000
BOOT_SEED = 0


def _boot_ci(x: np.ndarray, n: int = BOOT_N, seed: int = BOOT_SEED):
...
def derive_fig5(raw_dir: Path) -> dict:
    ...
    npz_path = raw_dir / "tok_subspan_attn.npz"
    meta_path = raw_dir / "tok_subspan_attn_meta.json"
    npz = np.load(npz_path)
    meta = json.loads(meta_path.read_text())
    conds = np.array(meta["conds"])

    def per_example(arm: str, span: str) -> np.ndarray:
    ...
    out: dict = {}
    for cond in FIG5_CONDS:
        mask = conds == cond
        out[cond] = {"n": int(mask.sum()), "spans": {}}
        for span in FIG5_SPANS:
            entry = {}
...
def derive_fig2_random_null(raw_dir: Path) -> dict:
...
def main():
    ...
    print("[precompute] wrote fig5_subspan_attention.json")

    fig2 = derive_fig2_random_null(raw_dir)
    (OUT / "fig2_random_null.json").write_text(json.dumps(fig2, indent=2))
    print(f"[precompute] wrote fig2_random_null.json ({fig2['source']})")
